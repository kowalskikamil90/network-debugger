    ...
    _timesEdit(new QSpinBox(this))
{
    ...
    _timesLabel->setText("How many packets to send?");
    _timesEdit->setFixedWidth(45);
    _timesEdit->setAlignment(Qt::AlignRight);

    // Limit the number of packets from 1 to 100
    _timesEdit->setRange(1, 100);
    _timesLayout->addWidget(_timesLabel);
    _timesLayout->addWidget(_timesEdit);
    _timesLayout->addStretch(1);

    /* Main layout for this tab */

    _vLayout->addWidget(_ipLabel);
    _vLayout->addLayout(_ipLayout);
    _vLayout->addLayout(_timesLayout);
